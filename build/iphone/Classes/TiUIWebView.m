...
static NSString * const kpaypalsampleJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_paypalsampleInjection
{
	if (pageToken==nil) {
	...
	[html appendString:kpaypalsampleJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
	...
	// attempt to make well-formed HTML and inject in our paypalsample bridge code
	   ...
	   baseURL:(NSURL*)baseURL
{
	...
	content = [[self class] content:content withInjection:[self _paypalsampleInjection]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
		...
		if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
			[NSURLProtocol setProperty:[self _paypalsampleInjection] forKey:kContentInjection inRequest:(NSMutableURLRequest *)request];
		}
		return YES;
	}
