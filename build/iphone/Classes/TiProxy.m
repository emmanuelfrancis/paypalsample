...
//Rare exceptions to indicate a bug in the _paypalsample code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_paypalsample = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_paypalsample]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.paypalsample.module"
